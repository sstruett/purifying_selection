...
def calc_density(my_file, options: dict={}):
    ...
    which_params = options.get("which_params", ("popsize", "selcoeff", "mutrate"))
    
    
    # extract the parameters
    N, s, U = extract_parameter_values(
        my_file, which_params=which_params
    )
    if s <= 0:
        s = -s
    else:
    ...
    try:
        ...
        time_, counts_ = np.unique(pd.read_pickle(my_file).to_numpy(), return_counts=True)
        simdat = pd.DataFrame({"time": time_, "count": counts_})
